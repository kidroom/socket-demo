  ...
  useEffect(() => {
    ...
    const handleNewMessage = (data: any) => {
      setMessages(prev => [
        ...
        {
          ...data,
          isCurrentUser: data.sender === user?.id
        }
      ]);
    };
  ...
  const groupedMessages = useMemo(() => {
    ...
    const allMessages = [
      ...(chat?.map(c => ({
        ...
        isCurrentUser: String(c.sender) === String(currentUserId)
      })) || []),
      ...messages.map(msg => ({
        ...msg,
    ...
    allMessages.forEach((message, index) => {
      ...
      const prevMessageTime = prevMessage?.timestamp ? new Date(prevMessage.timestamp).getTime() : 0;
      ...
      <div className="messageContainer">
        {groupedMessages.length > 0 ? (
          groupedMessages.map((group, groupIndex) => {
            const isSent = group.sender === currentUserId;
